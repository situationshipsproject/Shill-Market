...
export async function PATCH(
  ...
  if (!caller?.isSuperAdmin) {
    return NextResponse.json({ error: 'Only super admins can manage admin roles' }, { status: 403 })
  }

  const target = await prisma.user.findUnique({ where: { id } })
  if (!target) return NextResponse.json({ error: 'User not found' }, { status: 404 })

  // Protect: cannot demote other super admins
  if (target.isSuperAdmin && !caller.isSuperAdmin) {
    return NextResponse.json({ error: 'Cannot modify a super admin' }, { status: 403 })
  }

  const body = await req.json()
  const { isAdmin, tier, isVerified } = body
  ...
  const updated = await prisma.user.update({
    ...
    select: {
      id: true, username: true, displayName: true, isAdmin: true, isSuperAdmin: true, tier: true, isVerified: true,
    },
  })
